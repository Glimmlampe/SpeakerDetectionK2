...
//const char IPADRESSE[] = "localhost";
//const char IPADRESSE[] = "192.168.1.8"; 
//const char IPADRESSE[] = "134.103.120.149";
//const char IPADRESSE[] = "134.103.120.213";
//const char IPADRESSE[] = "134.103.120.123";
//const char IPADRESSE[] = "192.168.1.8";
//const char IPADRESSE[] = "127.0.0.1";
const char IPADRESSE[] = "134.103.120.233";

//const char IPADRESSE[] = "134.103.120.149";
const char OUTPORT[] = "8888";
const char INPORT[] = "8889";
...
void receive(int*, udp::socket*, udp::endpoint);
...
{
	// Set up the UDP socket to send the coefficients
/*	boost::asio::io_service io_service;
	udp::resolver resolver(io_service);
	udp::resolver::query query(udp::v4(), IPADRESSE, PORT);
...
{
	//Öffnen der UDP-Schnittstelle
	boost::asio::io_service io_service;
	udp::resolver resolver(io_service);
	udp::resolver::query query(udp::v4(), IPADRESSE, OUTPORT); //8888
	udp::endpoint receiver_endpoint = *resolver.resolve(query);

/*	boost::asio::io_service in_io_service;
	udp::resolver in_resolver(in_io_service);
	udp::resolver::query in_query(udp::v4(), IPADRESSE, INPORT); //8889
	udp::endpoint sender_endpoint;// = *in_resolver.resolve(in_query);
*/
	udp::socket socket(io_service);
//	udp::socket inSocket(in_io_service);

	socket.open(udp::v4());
//	inSocket.open(udp::v4());

	//Warte 1000ms (1 Sekunde)
	Sleep(1000);

	//Ausgabe im Fenster auf Desktop
	...
	//Ausgabe des Winkels über UDP

	const float PI = 3.1415926;
	float winkel_deg = m_fBeamAngle * (float)180.0 / PI;
	int sicherheit = 0;
	char noisedetection[32];

	//String initialisieren mit '\0'
	for (int i = 0; i < 32; i++)
		noisedetection[i] = '\0';

	//Setzen der Bool-Variable "sicherheit" bei confidence-Werten von derzeit >= 0.9
	if (m_fBeamAngleConfidence >= 0.9)
		sicherheit = 1;
	else
		sicherheit = 0;
	...
	int sendOn = 0;	 //0 setzen



	//boost::thread thread(receive, &sendOn, &socket, receiver_endpoint);
	...
	boost::array<char, 128> recMsg;

	while (sendOn != 1)
	{
		try
		{
			socket.receive_from(boost::asio::buffer(recMsg), receiver_endpoint, 0, ignored_error);
			//inSocket.receive(boost::asio::buffer(msg));
			//sendOn = msg[0];
			msg = recMsg.data();
			OutputDebugStringA(recMsg.data());
			OutputDebugStringA("\n");
			//sprintf_s(noisedetection, 32, "!%d%d%d%d", msg[0], msg[0], msg[0], msg[0]);
			//socket.send_to(boost::asio::buffer(noisedetection), receiver_endpoint, 0, ignored_error);
		}
	...
	{
		//String sequenzieren
		sprintf_s(noisedetection, 32, "#NOISEDETECTION#%d;%d;%.2f;", sicherheit, (int)winkel_deg, m_fBeamAngleConfidence);
		//String senden
		socket.send_to(boost::asio::buffer(noisedetection), receiver_endpoint, 0, ignored_error);
		if (ignored_error)
		{
			cout << "Error sending: " << ignored_error << endl;
		}
	}
}
/*
void receive(int *turnOn, udp::socket* socket, udp::endpoint receiver_endpoint)
{
	char msg[32];
	boost::system::error_code ignored_error;
	socket->receive_from(boost::asio::buffer(msg), receiver_endpoint, 0, ignored_error);
	socket->send_to(boost::asio::buffer(msg), receiver_endpoint, 0, ignored_error);
	if (ignored_error) 
	{
		cout << "Error sending: " << ignored_error << endl;
	}
